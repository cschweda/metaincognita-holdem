...
const OUTPUT_HTML = resolve(__dirname, '../app/public/analysis.html')
const OUTPUT_VUE = resolve(__dirname, '../app/pages/analysis.vue')
...
// Also generate a Vue page wrapper so /analysis works as a Nuxt route
const sampleHandsHtml = sampleHands.slice(0, 2).map((hand, i) => {
  const isOpen = i === 0 ? ' open' : ''
  return '<details class="mt-3"' + isOpen + '>' +
    '<summary class="cursor-pointer text-blue-400 hover:text-blue-300 text-xs font-semibold uppercase tracking-wider">Sample Hand ' + (i + 1) + '</summary>' +
    '<div class="bg-gray-950 rounded-lg p-4 mt-2 overflow-x-auto">' +
    '<pre class="text-[0.65rem] font-mono leading-relaxed whitespace-pre-wrap text-gray-400">' + escapeHtml(hand.trim()) + '</pre>' +
    '</div></details>'
}).join('\n')
...
const vueLines = [
  ...
  '<p class="text-gray-400 mt-2">Generated ' + dateStr + ' &middot; ' + totalHands.toLocaleString() + ' total hands &middot; $1/$2 Medium stakes</p>',
  ...
  '</div>',
  '',
  '<section class="mb-10"><h2 class="text-xl font-bold text-white mb-4">6-Player Table &mdash; ' + sim6p.hands + ' Hands</h2>',
  metricsGrid(sim6p),
  ...
  '</section>',
  '',
  '<section class="mb-10"><h2 class="text-xl font-bold text-white mb-4">8-Player Table &mdash; ' + sim8p.hands + ' Hands</h2>',
  metricsGrid(sim8p),
  '<div class="mt-6"><h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Observed vs Configured Stats</h3>',
  botStatsTable(sim8p.botStats),
  '</div>',
  ...
  '<p class="text-gray-500"><a href="/sample-hands.txt" class="text-blue-400 hover:text-blue-300 underline underline-offset-2" download>Download the full ' + HANDS + '-hand simulation file</a></p>',
  sampleHandsHtml,
  '</div></section>',
  '',
  '<footer class="border-t border-gray-800 pt-6 mt-10 text-center text-xs text-gray-600">',
  '<p>Generated by Hold\'em Simulator &middot; ' + dateStr + ' &middot; ' + totalHands.toLocaleString() + ' hands</p>',
  '</footer>',
  '</div></div>',
  '</template>',
]
writeFileSync(OUTPUT_VUE, vueLines.join('\n'), 'utf-8')
...
console.log(`\nStatic HTML: ${OUTPUT_HTML}`)
console.log(`Vue page:    ${OUTPUT_VUE}`)
