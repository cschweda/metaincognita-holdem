...
### Added
- **Composite exploit probe** (`scripts/composite-probe.ts`, `yarn probe:composite`) — a scripted hero plays Solid Sam's own SOLID baseline logic and deviates in exactly one way per run, so the delta against the untouched baseline is that one deviation's own EV. Built because the pure-degenerate battery above can't see a postflop leak on its own: a hero who jams any two cards pays so much blind tax that a real +18 bb/100 river leak vanishes inside a −300 bb/100 line. 16 cells cover this round's five fixes plus two documented residuals and two design-validation controls; `tests/composite-probe.test.ts` gates 14 of them in CI at 8 seeds × 30,000 hands per cell (adds roughly 4s to the suite's wall-clock, most of it overlapped by Vitest's file-level parallelism). This is a **secondary, coarse net**, not the primary regression guard — the deterministic static-sweep suites below hold that role, each independently shown to fail when its own fix is undone. The composite gate itself was shown (by reverting or config-neutralizing each fix) to trip cleanly for the whole 3-bet/4-bet family, `river-50`, `river-66` and the short-stack cell; three river cells moved hard in the same direction but landed inside a bound widened by their own measurement noise; the three turn cells did not respond meaningfully to an equivalent test (a multiway-dilution effect in this probe's 8-handed table, not a defect — see the test file's own header and the README Probe section). Every cell's delta carries wide, heavy right-tailed run-to-run variance even at 8 seeds × 30,000 hands — bounds were derived from four independent measurement batches per cell, not a single reading. `tests/exploit-probe.test.ts` also gained a 20bb depth (`[100, 25]` → `[100, 25, 20]`) to exercise the 12-25bb short-stack commit-rule band that neither existing depth touched.

### Fixed
- **A bot could bet its whole stack off as a pure bluff and not know it had, and out of position the river could not tell the blinds from the button** (`app/utils/botDecision.ts`, `app/pages/index.vue`, `holdem.config.ts` `strategy.postflop`/`strategy.river`) — found from a single reported hand: Ihil Pvey (`tiltMultiplier: 0.3`, the "almost untiltable" persona) led the river for 88% of a 17bb stack with king-high from the small blind on `3h 4s 8s Th 7d`, sat down behind $4, and wore a FULL TILT badge while doing it. Four independent defects, each measured before and after:
  - **Postflop bet sizes never consulted the stack.** `sizedBet`/`maybeOverbet` return a pot fraction, and `applyCommitRule` — which already promotes a committing raise to an all-in — was wired only into `decidePreflopAction`. In the reported spot 87.7% of river bluffs either overflowed the stack (13.8%, silently clamped by the betting engine) or left 1–5bb of unplayable dust behind (73.9%); a stub like that buys no more fold equity than a smaller bet and no more than the shove, and cannot threaten a later street. `applyPostflopCommitRule` now wraps the whole postflop path and promotes a bet to all-in when the leftover is dust — under a quarter of the pot the bet creates **and** under 6bb outright. Both tests are needed: the ratio alone shoved a half-pot turn bet that left 30bb behind in a 3-bet pot, reopening the `prem3bet-25-fj` composite cell at +6.1 bb/100 against its bound of 4; a BB floor alone would shove 6bb into a pot of 3.
  - **The river ignored position.** `ipAggBoost` (1.25 IP / 0.85 OOP) reached the c-bet, turn-barrel and river-barrel branches but not the river block that catches every bot that didn't bet the turn — which, because all three river branches return before the IP/OOP probe section, is the only place a river bluff can come from for such a bot. Measured at 40,000 seeded decisions in the reported spot, an SB lead and a BTN bet fired at byte-identical 9.24%. Now 6.27% SB against 9.22% BTN (0.68x) for Ihil Pvey, and the same discount holds for all 27 personas.
  - **The river ignored the board.** The same block had no texture or scare-card read, so it bluffed a straight-completing river as though the card had not come. The `riverBluffBoost`/`riverScareCard` computation is now hoisted out of the barrel branch and shared by both.
  - **A lead into the preflop aggressor took no donk discount.** On the flop and turn an OOP lead runs at 0.18 where the in-position probe runs at 0.40; the river had no equivalent, so a bot led it into the raiser as often as it bet in position. Personas carrying an explicit `donkBetFreq` lead at their own rate; a pro (`donkBetFreq: 0`, which in this engine means "use the rare fallback", not "never") now takes a `proDonkMod` of 0.45.
  - **The tilt badge reported a number no bot plays at.** `updateTilt` pins `severity` to 1.0 on a big-pot loss regardless of `tiltMultiplier`, and `PlayerSeat` rendered that raw value — so Ihil Pvey read FULL TILT while `applyTilt` was scaling him to 0.3. New `effectiveTiltSeverity(tilt, multiplier)` returns what the bot actually plays at, `applyTilt` routes through it so the two cannot drift, and the table binds the badge to it. Behavior is unchanged; only the label was wrong. The underlying separation was already correct and is now gated: on full tilt from the SB, Ihil Pvey's stack-off-with-air moves +1.07pp where Mike the Mouth moves +9.64pp and Hill Phellmuth +8.77pp.

  Balance: full suite 1132 tests / 53 files green, all 16 composite cells inside their bounds (the first cut of the commit rule tripped two and is guarded by its own regression test), and all 9 exploit-probe strategies still lose. Suites `tests/river-bluff-discipline.test.ts` (10 gates, 9 of which fail against the pre-fix engine) and `tests/position-commit-discipline.test.ts` (56 gates across every persona; 28 fail pre-fix).
- **The `--- DEAL ---` log labelled every hand with the previous hand's positions** (`app/pages/index.vue`) — `dealNewHand` wrote the deal log and then advanced the button, but the blinds and every bot decision run *after* that advance, so the log was one seat behind the hand it described. Surfaced by the per-hand console dump below, whose own seat table (built at hand end, from the advanced button) disagreed with it; the chip flow settled which was right — a player the deal log called UTG had posted exactly the big blind and folded. The button now advances before anything reads `positions`. Play was always correct; the log was not, and it feeds the in-app replay view and the PokerStars export as well as the console.
- **Per-decision console log** (`app/utils/devHandLog.ts`, `app/composables/useGameEngine.ts`) — one line per decision from every player, hero included, printed as it happens: street, position, hole cards, visible board, stack in bb, pot, SPR, amount to call with its pot odds, and the action taken with the share of stack and pot it commits. Anything that commits half a stack or more, goes all-in, or leaves under 6bb behind is flagged with `>>>` so a weird shove is findable by eye or by grep. Hooked into `applyAction`, the single choke point every seat's every action already funnels through. This does print a bot's hole cards while the hand is still live — unavoidable if the line is to explain the decision, and the reason this is a debugging build rather than a shipping one.
- **Per-hand console log** (`app/utils/devHandLog.ts`, `app/pages/index.vue`) — temporary debugging aid that prints one collapsed console group per hand: every seat's position, hole cards, stack in bb, invested, result, and both raw and effective tilt, followed by the full action line from deal to river. Called only from the hand-end handler, after the hand is decided, so an open console cannot be used to play the hand in front of you. Prints one flat `console.log` per hand as a single plain string — the first cut used `console.groupCollapsed` + `console.table`, which collapses to a line that is easy to miss among boot noise and, worse, cannot be copied out, since a console table is a rendered widget rather than text. It was also gated behind `import.meta.dev`, which deleted it from exactly the built artifact the game is normally played on. It now runs in every build, and a boot banner names itself on load so its absence immediately identifies a stale build. Console helpers: `holdemHands(n)` returns the last n hands as one pasteable string, `holdemCopy(n)` puts them on the clipboard, `holdemLog('off')` stops it without a rebuild. Visible to anyone who opens a console on a public deploy — delete the file and its two call sites before that matters. Verified in a live browser session across several hands, not by inspection.
- **River defense ignored bet size** (`app/utils/botDecision.ts`, `holdem.config.ts` `strategy.river`) — continuation on the river was a flat rate (roughly 46/46/45% against a 1/3-pot, pot-sized and 1.5x-pot bet, by static sweep) against a true minimum-defense frequency running from ~75% down to ~40% over that range, and unpaired hands / busted draws kept flop-style equity credit that only makes sense with cards still to come, so bluffs almost never fired (~95% of river bets were value). Continuation is now anchored to a locally-computed river true-defense frequency, split across multiway defenders and scaled by where the hand sits in its class; an unpaired hand or busted draw is scored as air on the river, so bluffs fire at the persona's configured rate (capped at 35%). Static sweep after: 1/3, pot and 1.5x pot now grade properly instead of sitting flat. Composite probe (8 seeds × 30,000 hands, 4 batches): the family moved from a pre-fix range of roughly +14 to +19 bb/100 down to roughly +3 to +14 at final code; `river-50`/`river-66` demonstrably trip their own CI bound on revert (proving the gate catches a regression here), while the rest of the family responded the same direction but stayed under bounds widened by their own noise. Two of these remain documented residuals rather than closed leaks, even at final settings: defense against a half-pot bet (`river-50`, still roughly +6 to +14) and the river value-overbet (`river-value-150`, down from +14.1 to roughly +5 to +11). Suite `tests/river-defense.test.ts`.
- **Short stacks played pure push/fold from 25bb down, and reused the shove range as the shove-calling range** (`app/utils/botDecision.ts`, `holdem.config.ts` `strategy.preflop`) — any raise below 25bb was itself a full shove or fold (bots never rebuy, so one lost pot ended a bot's normal short-stacked play for the rest of the session), and the reraise-jam defense floor, tuned for deep-stacked jams, saturated at a flat ~4% continue range from ~20bb down to the shove threshold. At 24bb, bots open-shoved 25.4% of hands and never raised normally. Push/fold now starts at 12bb; from 12bb to 25bb normal preflop logic runs with a commitment rule (a raise costing 40%+ of the stack is promoted to all-in), and jam-calling ranges widen continuously as the jam gets shorter, applying to both open-jams and reraise-jams. At 24bb, bots now never open-shove and raise normally 33.2% of the time. Composite probe: `pf-exploit`@24bb trips its CI bound when `pushFoldBB` is reverted to 25 (reproduces +8.97 bb/100 against a bound of 6); pre-fix it measured roughly +22.9, final code roughly −2.6 to +0.3. `nit-value`, the standing short-stack knife-edge, moved from a worst-case +9.5 bb/100 (24bb) to solidly negative at every depth. Suite `tests/short-stack.test.ts`.
- **Defense against a 3-bet or 4-bet didn't scale with its size** (`app/utils/botDecision.ts`, `holdem.config.ts` `strategy.preflop`) — the value/bluff 4-bet frequencies and both flat-call rates were identical whether the re-raise was 7.5bb or 60bb, so 3-betting a premium huge was free money once past the open-sizing defense. A continuous penalty now shrinks all three past a reference size (9bb for 3-bets, 25bb for 4-bets, or a multiple of the bot's own prior bet, whichever is larger) — unpenalized at or below the reference. Static sweep: continue rate vs a re-raise at 9/25/60bb moved from a flat ~19% to 19.1/12.6/2.4%. Composite probe: all three re-raise cells (`prem3bet-25-fj`, `prem3bet-50-fj`, `wide5-3bet-50-fj`) have negative canonical readings and each demonstrably trips its own CI bound on revert (e.g. `wide5-3bet-50-fj` reproduces +16.90 bb/100 against a bound of 2); pre-fix they measured roughly +5.3 to +16.2, and across four measurement batches per cell final code ranges roughly −16.5 to +1.0 across the family — comfortably negative in most readings, occasionally touching just positive for `prem3bet-50-fj` under the same heavy-tailed noise the river fix above discusses. One discontinuity is documented, not fixed: the size-penalty formula hands off to an older, separate jam-floor rule at the jam threshold; a probe cell built to target that exact seam (`reraise-cliff-bluff`) reads roughly −255 to −244 bb/100 — confirmed catastrophically unprofitable to exploit, but the seam itself remains. Suite `tests/reraise-size.test.ts`.
- **Big blind defense against a small open ignored its own pot odds** (`app/utils/botDecision.ts`, `holdem.config.ts` `strategy.preflop`) — the BB is already 1bb in the pot, so a 2-3bb open lays it better than 3:1, but the flat-call range didn't know it; static sweep measured 14.26% defense against a 2.5bb open (multiway). A BB-only multiplier now widens the flat-call range against opens at or below 3bb, fading out linearly by 6bb, moving defense to 22.99% against the same 2.5bb open. A review round found the first version also stacked on top of the already-wide heads-up branch, pushing the defend frequency past 100% for several loose personas (Loose Lucy, Bean-Robert Jellande and Wild Wendy folded 0% of hands heads-up); heads-up is now excluded (already wide by design) and a hard ceiling (0.90) backstops any future constant change. Suite `tests/reraise-size.test.ts`.
- **Turn defense had the same size-blindness the river had, found once the river was fixed** (`app/utils/botDecision.ts`, `holdem.config.ts` `strategy.turn`) — top-pair-class continuation was flat regardless of bet size (68.7/68.9/66.1% by static sweep against a 1/3-pot, pot-sized and 1.5x-pot bet); with preflop continuing widened elsewhere this round, a hero betting a third of pot on both the turn and river whenever checked to became the single largest number in the composite battery (roughly +11.9 bb/100 — a mid-round reading of +19.0 did not reproduce; it was taken while the BB-defense fix above's first version, later corrected, was still active and had itself been widening BB calling ranges enough to let more hands reach the turn and river than final code does — turn itself was unfixed in both readings). Turn continuation is now anchored to the same true-defense-frequency mechanism as the river, with wider floors (a card is still to come). Static sweep after: top-pair defense against 1/3, pot and 1.5x pot now grades 90.5/61.7/47.7% instead of sitting flat. The composite probe's isolated turn cells (`turn-33`, `turn-river-33`, `turn-value-150`) did not demonstrate a trip under an equivalent neutralization test — `tests/turn-defense.test.ts`'s static sweep, not the composite gate, is this fix's real regression guard (see Added, above). One residual is documented, not closed: the shipped turn constants are deliberately wider than the river's own vetted ones, and the value-direction check (`turn-value-150`) reads roughly +0.8 to +14.6 bb/100 across four measurement batches. Suite `tests/turn-defense.test.ts`.
- **Preset switch in the bot configurator could inherit the previous persona's betting tendencies** (`app/components/SetupScreen.vue`) — `applyPreset` copied only 7 of a bot's 12 strategy fields, so switching a seat away from, say, Calling Carl (`donkBetFreq: 0.22`, `limpFreq: 0.65`) to a pro left those fields attached to the new persona; `generateDefaultBots` had the correct mapping but as a second, separately-drifting copy. Both call sites now share one `botStrategyFromPreset` (`app/utils/botConfig.ts`), which assigns every field unconditionally — `undefined` included, for a field the new preset doesn't define — so a switch can no longer leak a stale value. Suite `tests/bot-config.test.ts`.
- **Hero session read broke on a name-sharing bot** (`app/pages/index.vue`) — VPIP, fold-to-3-bet, fold-to-c-bet and aggression were derived by scanning the hand log with `line.includes(heroName)`, so a hero named e.g. "Sam" absorbed every "Solid Sam …" line; the same substring test drove the bots' tilt-`participated` flag, and a hero named e.g. "Wild" or "Mike" would have absorbed a longer bot name the other direction ("Wild Wendy", "Mike the Mouth"). `actedInLine` (`app/utils/heroRecord.ts`) now anchors on the actor at the start of the line, immediately followed by one of the engine's five action verbs, closing both collision directions; the setup screen also rejects a blank or whitespace-only hero name, falling back to the configured default. Suite `tests/hero-record.test.ts`.
- **Every GitHub repository link pointed at the wrong, 404ing repo, and 20 changelog versions had no link at all** (`CHANGELOG.md`, `app/pages/index.vue`, `package.json`) — 21 links (20 in this changelog's own version headings, 1 in the in-app GitHub footer) were templated from the project's old name, `cschweda/holdem-simulator`; the real remote is `cschweda/metaincognita-holdem`. Fixed everywhere, `package.json` now declares `repository`, and 20 previously-undefined version headings (0.11.1 through 0.19.0) got their compare-link definitions. Suite `tests/links.test.ts`.

### Removed
- **`sessionMemory.shortStackThreshold`** — dead config field, superseded by `strategy.preflop.pushFoldBB` / `shortStackCeilingBB` (see Fixed, short stacks); nothing read it, and its pinning test is gone with it.

### Documentation
- README: Preflop/Postflop Decision Flow sections updated for the short-stack, re-raise-size and blind-defense changes above; the Exploit Probe section's results table refreshed to this round's own 8 seeds × 30,000-hand protocol, and a new Composite Probe section added with the per-cell before/after table. Security → Audit Log gained a Round 8 entry. Test counts refreshed: 983 tests / 43 files → 1,132 tests / 53 files.
...
- **Browser simulator test** (`tests/simulate-browser.test.ts`) — the `/analysis` simulator relied on a Nuxt auto-import for `chenScore` and could not load under vitest or vite-node, leaving it the only table-read consumer with no automated coverage. Explicit import; the new suite runs a seeded 40-hand session end to end and pins seed determinism.
- **Wider CI exploit-probe gate** — two new degenerate hero strategies from the Round 7 red-team, `steal-fold` (open CO/BTN any two, fold to 3-bets, one c-bet) and `donk-33` (call opens, bet ⅓ pot whenever checked to), the two that came closest to break-even; and the whole battery now also runs at 25bb (`runStrategy(..., depthBB)`, `yarn probe all 10000 <seed> 25`), since short stacks change the jam/call maths and the gate only ever measured 100bb. The scripted hero can now see the street-sliced board, its position and whether it was the preflop raiser. All nine strategies lose at both depths (fit-or-fold added with table reads).
- **Round 6 security audit** — whole-app red/blue sweep (hidden-information fairness, injection/persistence, resource lifecycle) covering the post-Round-5 code: career mode, Nemesis, and the keep-alive game page. Ten findings — six fixed in-round (see Fixed below), both remaining Round 6 findings were closed in this release (the thinking-insight pill is gated behind the study toggle, and the static analysis report is gone), two accepted (career refresh-refund, client-side card visibility). Full log in README → Security → Audit Log. New suites: `tests/information-hygiene.test.ts` (bots can never see future streets) and `tests/persistence-guards.test.ts` (hostile localStorage); the vitest `~` alias now points at `app/` (Nuxt 4 srcDir semantics), making stores testable. Suite: 879 → 896 tests.
...
### Fixed
- **Outs were overcounted** (Round 7 #1, `handAnalysis.ts`) — `totalOuts` summed overlapping draws, a pocket pair's "overcards" were counted on top of its set outs, and any straight window missing an edge rank was called open-ended, so A-K-Q-J and A-2-3-4 got 8 outs instead of 4. A pocket pair over the board showed 10 outs and 38% by the river (true 2 and 8%); QQ on 7-7-2-3 showed 12 (4). Every draw now carries its physical out cards, the total is the union of distinct cards, made straights/flushes are not draws, and the panel lists the exact out cards from the same source. Commentary uses the same de-duplicated total. New suite `tests/analysis-ground-truth.test.ts`.
- **Preflop equity was a Chen-score bucket table** (Round 7 #2) — suited broadways and connectors were scored like pairs (AKs 77% vs 67% true, 98s 60% vs 51%). Preflop now runs the same 1,000-runout Monte Carlo pass as postflop; every checked hand lands within ±2 points of published values.
- **Bot thinking pill revealed the acting bot's hand mid-hand** (Round 6 #6) — the insight (Chen score, made hand, draws) was built precisely while the hero was still in the hand. It is now built only once the hero is out, or when the new setup toggle **Show Bot Thinking: Always** is on — a study aid that knowingly reveals hidden cards. `canRevealBotThinking` in `app/utils/thinkingInsight.ts`, pinned by `tests/thinking-insight-gate.test.ts`.
- **Action recommendation ignored the price** (Round 7 #6, `handAnalysis.ts`) — `recommend()` saw only a facing-a-bet flag, so a 36%-equity "CALL" was advised against a third-pot bet and a 3× overbet alike, sometimes contradicting the pot-odds verdict beside it. `analyzeHand` now takes pot and call amounts, `potOddsNeeded` is real, and the recommendation folds when equity is short of the price (strong draws get a few points of implied-odds slack). Suite `tests/recommendation-price.test.ts`.
- **TV-mode commentary foreshadowed the undealt runout** (Round 7 #3, `useCommentary.ts`) — the flop/turn "I've seen the future" quips read the full board and fired only when a live player would improve by the river, a real signal for a TV-mode hero. Both peeks now go through `foreshadowAllowed` (`commentaryStrategic.ts`), which refuses while the hero is still in the hand. Suite `tests/commentary-hygiene.test.ts`.
- **Opponent HUD showed persona config, not observed play** (Round 7 #4, `index.vue`) — the per-bot VPIP/PFR/AF were the bot's configured numbers with a hardcoded 25 hands and an invented WTSD, so the "Nit — 3-bet liberally" reads printed from hand one. New `app/utils/observedStats.ts` parses per-bot VPIP / PFR / AF / WTSD from the session's recorded action logs with standard denominators; the panel's 10-hand minimum now gates the reads for real. Suite `tests/observed-stats.test.ts`.
- **Stats page counted uncontested wins as showdown wins** (Round 7 #5, `useStatsData.ts`) — "Showdown Rate" and "Won at Showdown" now count only hands that reached showdown (five-card board, two or more unfolded players).
- **Live-game clairvoyance** (Round 6) — the browser-sim fix below missed the live table: `index.vue` and `replay.vue` still passed the full 5-card runout into every bot decision context, so live bots evaluated flop decisions with their final 7-card hand and ran turn/river-card logic on future cards — the CI difficulty gate was calibrated on fair bots while the shipped game played cheating ones. Community is now street-sliced at the single `decideBotAction` entry point (no caller can hand a bot future cards) and both call sites pass `visibleCommunity`. Contract pinned by `tests/information-hygiene.test.ts`; the seeded probe battery is byte-identical on both recorded seeds, confirming the probe path was already fair and only the live table changed.
- **Keep-alive lifecycle** (Round 6) — navigating away mid-hand no longer leaves the game playing itself in the background: route deactivation pauses the engine (bots hold at their next "thinking" sleep) and activation resumes it. Stale bot timers can no longer act into a freshly dealt hand or throw after `backToSetup()` empties the table — the engine gained an epoch guard, tracked/cancellable sleeps, and cleanup on every teardown path (setup, rebuy, timeout, career leave, replay restart/unmount). Career "Play" now works from a cached table page (`pendingSession` consumed in `onActivated`; previously the 2nd+ session debited the buy-in and never started), and the f/c/r hotkeys go inert off the game page (pressing `f` on `/stats` used to fold the hero in the background game).
- **Persistence hardening** (Round 6) — career and nemesis loads now shape-validate and repair (`sanitizeCareerState` / `sanitizeHeroModel`) instead of trusting any `version: 1` JSON, so a tampered or corrupt localStorage payload can no longer brick `/career` in a render-throw loop, throw inside live bot decisions, or strand `/stats` on its loading spinner. All session-stats writes go through a warn-once quota-guarded writer (a full localStorage no longer aborts the tail of `endHand`), commentary sliders NaN-clamp their stored values, and `/replay-hand?hand=%` shows a parse error instead of crashing the mount. 14 new tests in `tests/persistence-guards.test.ts`, including hostile-storage store loads.
- **Phone layout** — the game column shrink-wrapped below the `xl` breakpoint (`items-start` parent, no explicit width), collapsing the table to ~315px at any sub-1280px viewport — with fixed-size cards (the community row alone was 432px) and a non-wrapping top bar forcing horizontal scroll, both phone orientations were unplayable. The column now takes full width, all three card sizes scale down below 640px at the definition site (`PlayingCard.vue`, so every usage inherits), seat nameplates/gaps compress, the top bar wraps, and a CSS `clamp()` keeps the 3-and-9-o'clock seats on-screen (a no-op at desktop widths). Verified at 390×844 portrait and 844×390 landscape.
...
### Removed
- **Static analysis report and its generator** (Round 6 #7) — `public/analysis.html` loaded `cdn.tailwindcss.com`, which the site's own CSP blocks, and was a stale snapshot of the live `/analysis` page; `scripts/generate-analysis.ts` shelled out to a tool that isn't installed and would have overwritten the live page with a static one. Both deleted with the Netlify redirect; `sample-hands.txt` stays and the README shows how to refresh it. Pinned by `tests/static-report-retired.test.ts`.
...
### Performance
- **One storage write per hand** (Round 7 #11, `useSessionStats.ts`) — the deep watcher that re-walked the entire hand history on every change and re-serialized it a second after each explicit save is gone; persistence is explicit on session start (new: a fresh session is saved before its first hand), on each recorded hand, and on tab close. Suite `tests/session-stats-writes.test.ts`. Lazy-loading the 32 KB-gzipped quip table was considered and skipped: commentary defaults to on, so nearly every session loads it anyway.
...
### Security
- **Toolchain upgraded within its ranges** (Round 7 #11) — Nuxt 4.4.2 → 4.5.2, Nuxt UI 4.6.0 → 4.11.0, Vitest 4.1.2 → 4.1.11, vue-tsc 3.3.7 → 3.3.11 and friends. `yarn audit` goes from 129 advisories (4 critical / 78 high / 38 moderate / 9 low, all build-time packages) to 1 low. Typecheck, the full suite and the production build are green; the bundle still contains no `eval` / `new Function`.
- **CSP tightened** (Round 7 #8, `netlify.toml`) — `'unsafe-eval'` dropped (the production bundle has no `eval` / `new Function`), `object-src 'none'`, `base-uri 'self'` and `form-action 'self'` added, and a one-year `Strict-Transport-Security` header. `'unsafe-inline'` stays for the Vue runtime; the iconify origin stays as an icon-bundle fallback.
- **Round 7 audit** — whole-app analysis across four lanes (bot hidden-information and strategic leaks, analysis correctness, efficiency, security), every finding was worked through in the same round (see the README audit log). Headline: the analysis panel overcounts outs (a pocket pair over the board shows 10 outs / 38% instead of 2 / 8%) and its preflop equity table scores suited broadways like pairs (AKs 77% vs 67% true); the opponent HUD prints configured persona stats with an invented WTSD; TV-mode commentary foreshadows the undealt runout; the engine's "opponent reads" counters are never incremented. No strategic leak: twelve new degenerate strategies lose on both seeds at 25/100/250bb. Security re-verified clean; the CSP dropped 'unsafe-eval' and deliberately kept the iconify origin as an icon-bundle fallback.
...
### Documentation
- README: test counts refreshed (983 tests / 43 files), Security audit log Round 5 added.
- README: exploit-probe section rewritten — corrected run command (`yarn probe all 10000`), fresh fixed-depth results for all nine strategies at 100bb and 25bb, and a post-mortem of the nit-value leak replacing the old "+64 bb/100 is fine, as in real life" rationalization.
...
[Unreleased]: https://github.com/cschweda/metaincognita-holdem/compare/v0.20.0...HEAD
[0.20.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.19.0...v0.20.0
...
[0.10.2]: https://github.com/cschweda/metaincognita-holdem/compare/v0.10.1...v0.10.2
[0.10.1]: https://github.com/cschweda/metaincognita-holdem/compare/v0.10.0...v0.10.1
[0.10.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.9.2...v0.10.0
[0.9.2]: https://github.com/cschweda/metaincognita-holdem/compare/v0.9.1...v0.9.2
[0.9.1]: https://github.com/cschweda/metaincognita-holdem/compare/v0.9.0...v0.9.1
[0.9.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.8.1...v0.9.0
[0.8.1]: https://github.com/cschweda/metaincognita-holdem/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.7.1...v0.8.0
[0.7.1]: https://github.com/cschweda/metaincognita-holdem/compare/v0.7.0...v0.7.1
[0.7.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.6.0...v0.7.0
[0.6.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.5.1...v0.6.0
[0.5.1]: https://github.com/cschweda/metaincognita-holdem/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.3.1...v0.4.0
[0.3.1]: https://github.com/cschweda/metaincognita-holdem/compare/v0.3.0...v0.3.1
[0.3.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/cschweda/metaincognita-holdem/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/cschweda/metaincognita-holdem/releases/tag/v0.1.0
