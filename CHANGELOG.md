...
### Added
- **Multi-seat exploit probe** — `runStrategy(..., heroSeats)` and a sixth CLI argument seat several copies of a strategy (the pro lineup fills the rest; bb/100 is per seat), because one hero cannot make an eight-handed table abnormal. Two new gate cells at 100bb: four calling stations (the station read fires in most windows; −669 bb/100 per seat) and four fit-or-fold seats (−563). The sweep also showed the weak-tight read never fires in any composition (fold-heavy seats make the table showdown-heavy, and the pros' own bets keep passivity below threshold), so that branch is documented as latent.
- **Table reads** — bots notice a calling-station table (check-heavy, everything reaches showdown) and value-bet thinner / bluff the river less, or a weak-tight table (check-heavy, folds before showdown) and probe more. Public signals only, over a rolling 30-hand window (`app/utils/tableReads.ts`), thresholds calibrated outside a normal pro table's range, three bounded multipliers in `config.strategy.tableReads`. The live engine, both simulators and the exploit probe feed the same tracker: `station` exercises the calling-station read end to end in CI (bots value-bet it thinner — −1,080 → −1,142 bb/100 at 100bb, −552 → −565 at 25bb); the weak-tight read is unit-tested (each multiplier moves exactly its knob) and calibrated (a fold-everything hero produces a read in under 5% of windows), but no single-hero probe strategy can make an eight-handed table check-heavy enough to trigger it, so the gate gains `fit-or-fold` — a distinct degenerate line — and a calibration test, not an end-to-end measurement of the weak-tight branch. Replaces the dead "opponent reads" plumbing removed earlier in Round 7. A same-day fix wave then corrected three bugs a whole-branch review caught: the thin-value boost was clipped by the frequency cap for aggression ≥ ~1.15 (now applied after the cap, so the full ×1.4 holds — the source of the station-cell shift above), the probe/stab boost never reached the river (now applied to the river non-raiser air bluff-bet), and the showdown reads could fire on too few flops (now held off below `minFlops`, 5).
