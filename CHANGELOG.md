...
- **Per-hand console log** (`app/utils/devHandLog.ts`, `app/pages/index.vue`) — temporary debugging aid that prints one collapsed console group per hand: every seat's position, hole cards, stack in bb, invested, result, and both raw and effective tilt, followed by the full action line from deal to river. Called only from the hand-end handler, after the hand is decided, so an open console cannot be used to play the hand in front of you. First cut was gated behind `import.meta.dev`, which meant it vanished from exactly the built artifact the game is normally played on — it now runs in every build, with a runtime off switch (`localStorage.holdemHandLog = 'off'`) that needs no rebuild. It is therefore visible to anyone who opens a console on a public deploy: delete the file and its single call site before that matters. Verified in a live browser session, not just by inspection.
