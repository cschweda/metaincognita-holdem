...
- **Dev-only per-hand console log** (`app/utils/devHandLog.ts`, `app/pages/index.vue`) — temporary debugging aid that prints one collapsed console group per hand: every seat's position, hole cards, stack in bb, invested, result, and both raw and effective tilt, followed by the action line. Gated behind `import.meta.dev` so it never reaches a built site, and called only from the hand-end handler after the hand is decided, so an open console cannot be used to play the current hand. Delete the file and its single call site to remove it.
