...
import type { SessionRow, HandRow } from '~/composables/useStatsData'
...
// ─── UI State (modals, selections — page-only) ──────────
...
async function copyHandToClipboard(h: HandRow) {
  const text = toPokerStarsFormat(h, getStakeFromLevel(h.stake_level))
...
async function handleDeleteAllData() {
  await deleteAllData()
  showDeleteAllModal.value = false; deleteAllConfirmText.value = ''
}
async function handleDeleteSession(sessionId: string) {
  await deleteSession(sessionId)
  showDeleteSessionModal.value = false; deleteSessionTarget.value = null
}
async function handleDeleteHand(handId: string) {
  ...
  showDeleteHandModal.value = false; deleteHandTarget.value = null
}
      ...
      <template #body>
        <div class="space-y-4 p-6">
          ...
          <div class="flex justify-end gap-2 pt-2">
            ...
            <UButton
              color="error"
              @click="handleDeleteSession(deleteSessionTarget!.id)"
            >
              Delete Session
            </UButton>
          </div>
      ...
      <template #body>
        <div class="space-y-4 p-6">
          ...
          <div class="flex justify-end gap-2 pt-2">
            ...
            <UButton
              color="error"
              @click="handleDeleteHand(deleteHandTarget!.id)"
            >
              Delete Hand
            </UButton>
          </div>
