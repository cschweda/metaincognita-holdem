...
// Chorman serious ratio — 0 = all jokes, 100 = all strategy. Default 30 (mostly funny, some insight)
...
// ─── Chorman strategic observations (serious poker analysis) ─────
// These are generated dynamically with context, not from a static pool.
// Used when normanWantsToBeSerious() returns true.

function normanStrategicFlopObs(heroCards: [Card, Card] | null, community: Card[], players: PlayerState[]): string | null {
  ...
  if (draws.length > 0) {
    ...
    const hitPct = Math.round(totalOuts * 2) // rule of 2 for next card
...
function normanStrategicActionObs(playerName: string, actionType: string, amount: number, pot: number, heroCards: [Card, Card] | null, community: Card[]): string | null {
  const obs: string[] = []
...
function normanStrategicShowdownObs(heroWon: boolean, pot: number, heroCards: [Card, Card] | null, community: Card[]): string | null {
