...
onMounted(async () => {
  ...
  // Listen for auth changes (e.g., GitHub redirect back)
  ...
  <div class="relative">
    ...
    >
      ...
      <span
        ...
        :class="checking ? 'text-yellow-400' : connected ? 'text-green-400' : 'text-red-400'"
      >
        <template v-if="checking">Connecting...</template>
        <template v-else-if="isGitHub">{{ displayName }}</template>
        <template v-else-if="connected">Anonymous</template>
    ...
    >
      <div class="p-3 border-b border-gray-800">
        <div class="text-xs text-gray-400">
          ...
          <template v-else-if="connected">
            Anonymous session — stats are browser-local.
          </template>
          <template v-else>
            Not connected to Supabase.
          </template>
        </div>
        <div v-if="isGitHub" class="text-[0.6rem] text-green-500/60 mt-1">
      ...
      <div class="p-1.5">
        <template v-if="!isGitHub && connected">
          <button
